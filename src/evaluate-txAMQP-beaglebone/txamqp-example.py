...
def consume1(result, topics):
    ...
    yield channel.basic_consume(queue=QUEUE_NAME, no_ack=True, consumer_tag='qtag')
    queue = yield connection.queue('qtag')
    while True:
        msg = yield queue.get()
        msgnum += 1
        print " [%04d] Received %r from channel #%d" % (msgnum, msg.content.body, channel.id)
    returnValue(result)
...
def check_cmd_line(options, args, abort):
    ...
    if options.message is not None and (options.example not in PUBLISH_EXAMPLES):
